...
- **Fase 9 (completada):** lista de pendientes compartida y ordenable — cola única del hogar en `household_watchlist_order` (idéntica para ambos miembros; el botón "Me gusta" y la decisión de _qué vemos_ ahora comparten orden), reordenamiento atómico por RPC `reorder_household_watchlist` con validación del conjunto pendiente completo, drag & drop (dnd-kit, mobile-first) con fallback de flechas y teclado, filtro tipo local que conserva el orden, badge de posición en tarjetas, triggers que mantienen la cola al entrar/salir de pendientes, tests RLS (`08_watchlist_order.sql`) y unitarios de ordenamiento.
